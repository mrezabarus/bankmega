...
class Pic_db extends CI_Model {
	...
	*/
	...
	{	
		if(!empty($_SESSION['picArea'])){
			...
			$id_kursus = "and id_kursus = '". $picArea ."'";
		}
		else{
			$id_kursus = " ";
		}
	...
	*/
	...
	function moodle_export($key, $rowData)
	{
		...
		$result 	= $this->moodle_ExportBody($rowData); 	//CurveExportBody_rev2		
	...
	function moodle_export_all($key, $rowData)
	{
		...
		$result 	= $this->moodle_ExportBody_all($rowData); 	//CurveExportBody_rev2		
		$fileName = $key['fileName'] .'.xls';
		$data['fileName'] = $fileName;
		// file_put_contents( "./assets/download/" . $fileName, $result);
	...
	*/
	...
	function CurveExport_rev2($key)
	{
		...
		$rowData 	= $this->Pic_db->ReportDetail_rev2($key); // get per batch 
		$result 	= $this->Pic_db->CurveExportBody_rev2($rowData); 			
