<?php
	...
	//Load data
		...
		function Report_Content(){
			...
			$defy=$y=$this->GetY(); 
			$js=cmsDB();
			$js->query("SELECT *,DATE_FORMAT(date_of_birth, '%m-%d-%Y') AS date_of_birth1 
			            FROM tbl_jobseeker 
			...
			$this->Cell(40,5,$mar_status,1,0,'C',0);
			...
			if($family->recordCount()){
				...
				$father_last_edu=$family->row("sex");
				...
				$mother_last_edu=$family->row("sex");
				...
				//Brother& Sister
				for ($i=1;$i<=$family->recordCount();$i++) {
					$family->next();
			...
			$this->Cell(10,5,$father_last_edu,1,0,'C',0);
			$this->Cell(30,5,$father_place_of_birth.' / '.$father_date_of_birth1,1,0,'C',0);
			$this->Cell(30,5,$father_last_edu,1,0,'C',0);
			$this->Cell(0,5,$father_job,1,1,'C',0);
			
			$this->Cell(30,5,$mother_name,1,0,'C',0);
			$this->Cell(30,5,"Ibu - Mother",1,0,'C',0);
			$this->Cell(10,5,$mother_last_edu,1,0,'C',0);
			$this->Cell(30,5,$mother_place_of_birth.' / '.$mother_date_of_birth1,1,0,'C',0);
			$this->Cell(30,5,$mother_last_edu,1,0,'C',0);
			...
			for ($i=1;$i<=6;$i++){
				...
				$this->Cell(10,5,$bro_sex[$i],1,0,'L',0);
				$this->Cell(30,5,$bro_pob[$i].' / '.$bro_date_of_birth1[$i],1,0,'L',0);
				$this->Cell(30,5,$bro_job[$i],1,0,'L',0);
			...
			for ($i=1;$i<=4;$i++){
				...
				$this->Cell(10,5,$child_sex[$i],1,0,'L',0);
			...
			$this->MakeColDblRow($x+$np,$y,10,8,'Kota','City');$np+=10;
			$this->MakeColDblRow($x+$np,$y,30,8,'Jurusan','Major In');$np+=30;
			$this->MakeColDblRow($x+$np,$y,30,8,'Tahun','Years');$np+=30;
...
//			$this->MakeColumn($x+$np,$y,20,10,'Nama1','Name1');
			...
			for ($i=0;$i<count($inTp);$i++) { 
				...
				$this->MakeFillCol($x+$np,$y,10,5,$sd_city[$l]);$np+=10;
				//$this->MakeFillCol($x+$np,$y,0,5,$sd_major[$l]);$np+=10;
				$this->MakeFillCol($x+$np,$y,30,5,'');$np+=30;
				$this->MakeFillCol($x+$np,$y,30,5,$sd_year[$l]);$np+=30;
...
//			$this->MakeColumn($x+$np,$y,20,10,'Nama1','Name1');
			...
			for ($i=0;$i<3;$i++) {
				$this->Cell(35,5,$job_company[$i],1,0,'C',0); 
				$this->Cell(35,5,$job_duration[$i],1,0,'C',0); 
				$this->Cell(35,5,$job_title[$i],1,0,'C',0);
			...
			$this->MakeColDblRow($x+$np,$y,40,8,'Jabatan','Title');$np+=40;
...
When & what did you apply for?',
			...
			$job->query("SELECT * FROM tbl_jobseeker_addtional_info WHERE question <>'reading_freq' AND js_id='".$this->js_id."'");
			if($job->recordCount()){
				for ($l=0;$l<=$job->recordCount();$l++){
					$job->next();
					if ($job->row("answer_status")=='y') {$info_y[$l]='y';}
					else{$info_n[$l]='n';}
					
					$info_notes[$l]=$job->row("answer");
				}
			}
			...
			$job->query("SELECT * FROM tbl_jobseeker_questionare WHERE js_id='".$this->js_id."'");
			if($job->recordCount()){
				for ($i=0;$i<=$job->recordCount();$i++){
					$job->next();
					$question[$i]=$job->row("answer");
				}
			}//if($job->recordCount())
			...
			$this->Cell(0,5,"Pelamar / application",0,1,'C',0);
			$this->ln(10);
			$this->SetX($x+120);
