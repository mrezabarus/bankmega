	...
	?>
       ...
       <tr>
		 ...
		 <?
				if(isset($_GET["selregion"])){
						$branch_name=cmsDB();
						$strsql ="select * from tbl_branch where region_id=".$_GET["selregion"]." and branch_id in (".$_SESSION["ssbranch_id" . date("mdY")].") order by branch_id DESC";
       ...
       <tr>
		...
		$position=cmsDB();
		/*$strsql = "select * from tbl_position where position_id in (
    select distinct(tbl_position.position_id) as idpos from tbl_position inner join tbl_branch_mpp_apply
				on tbl_position.position_id=tbl_branch_mpp_apply.position_id
				where tbl_branch_mpp_apply.branch_id =" .$_GET["selbranch"] .")";
		*/
		$strsql = "select * from tbl_position order by position_name asc";
