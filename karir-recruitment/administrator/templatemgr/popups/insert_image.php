...
function onOK() {
  ...
  for (var i in fields) {
	...
	if(i==0){
		param[id] = "<?=$FJR_VARS["www_img_url"]?>" + el.value;
	}else{
    	param[id] = el.value;
	}
	//alert(param[id]);
...
function onPreview() {
  ...
  var url = "<?=$FJR_VARS["www_img_url"]?>" + f_url.value;
  if (!url) {
...
<table border="0" width="100%" style="padding: 0px; margin: 0px">
  ...
  <tr>
	...
	<SELECT name="URL" id="f_url" style="width:55%" onchange="return onPreview();">
		...
		<?
		$d = dir($FJR_VARS["www_img_path"]); 
		while (false !== ($entry = $d->read())) { 
			if($entry <> "." && $entry <> ".."){
				if(is_dir($FJR_VARS["www_img_path"].$entry)){
					$d2 = dir($FJR_VARS["www_img_path"].$entry);
