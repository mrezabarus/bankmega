...
<html>
<head>
<title><?=$FJR_VARS["admin_title"]?> -  Template Editor</title>
...
if(isset($_GET["finish"])){
	...
	$content = str_replace('["www_title"]','|"www_title"|',$content);
	$content = str_replace("['www_title']",'|"www_title"|',$content);
	$content = str_replace($FJR_VARS["www_img_url"],'[$FJR_VARS|"www_img_url"|]',$content);
	$content = str_replace("[","<?=",$content);
	$content = str_replace("]","?>",$content);
	$content = str_replace('|"www_title"|','["www_title"]',$content);
	$content = str_replace('|"www_img_url"|','["www_img_url"]',$content);

	//echo "<script>opener.document.".$_GET["function"].CONTENT.value=\"". jsFormat($content)."\";window.close();</script>";
	//die();
}
if(isset($_GET["function"])){
	
	$strSQL = "SELECT template_id,template_type,template_group,custom_charset,ttemplate.language_id,
	                  tlanguage.charset,isindex,ttemplate.display_name as tempDName,
					  tlanguage.display_name as langDName,edition_id 
			   FROM ttemplate 
			   LEFT JOIN tlanguage ON ttemplate.language_id = tlanguage.language_id 
			   WHERE template_id = '".$_GET["pgid"]."'";
...
function highlight() {
  editor.surroundHTML('<span style="background-color: #FFFFFF">', '</span>');
}
</script>
