...
function editTableWin(num) {
	...
	vOut = window.showModalDialog(inc + "tableprop.htm",vIn,"dialogWidth:407px; dialogHeight:296px;help:0;status:no;");
	if (vOut != null) {
		for ( elem in vOut ) {
			switch (elem) {
...
function bs_hyperlinkwin(num) {
  ...
  vOut = parent.showModalDialog(inc+"link.php?seed=<?=mktime();?>",vIn,"dialogWidth:600px; dialogHeight:550px;help:0;status:no;");
  ...
  if (vOut != null){
		...
		iclass = vOut["class"]!=null?vOut["class"]:"";
...
function set_tbstates(num) {
	try {
		...
		for(var i=0;i<bs_tbar.all.length;i++) {
			...
			else if(cid==DECMD_HYPERLINK||cid==DECMD_SPECIAL_CHARS||cid==DECMD_HR||cid==DECMD_SAVE||cid==DECMD_FILE) {
	   		if(bs_sourceview[num]) {
					if(pbtn.state!=0) {
							pbtn.className = "disabled";
							pbtn.state = 0;
		...
		if(oSel=eval('document.all.oQuickFontSize'+num)){
			if(DHTMLSafe.QueryStatus(DECMD_SETFONTSIZE)!=DECMDF_DISABLED&&DHTMLSafe.DOM.selection.type!="control"&&!bs_sourceview[num]) {
				if(oSel.disabled) oSel.disabled=false; 
				fs=DHTMLSafe.ExecCommand(DECMD_GETFONTSIZE);
				if(fs!=null&&fs!="") {
					for(i=0;i<oSel.options.length;i++) {
						if(oSel.options[i].name=="bs_fs"+fs&&!oSel.options[i].selected)
							oSel.options[i].selected=true;
					}
				}
				else {
		...
		if(oSel=eval('document.all.oQuickFormat'+num)){
			if(DHTMLSafe.QueryStatus(DECMD_SETBLOCKFMT)!=DECMDF_DISABLED&&DHTMLSafe.DOM.selection.type!="control"&&!bs_sourceview[num]) {
				if(oSel.disabled) oSel.disabled=false;
