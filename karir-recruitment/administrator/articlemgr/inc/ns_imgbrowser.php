...
<head>
  ...
  <!--
	function select_file(name) {
		parent.preview.location = "<?=$imgurl.$op?>" + name;
		parent.properties.document.IMGPROP.iURL.value = "<?=$imgurl.$op?>" + name;
	}
  //-->
  </SCRIPT>
</head>
...
<? if ($op!="/"&&$op!="") { ?>
	<tr>
		<td><table cellpadding="0" cellspacing="0" border="0">
			<tr>
				<td nowrap><a title="<?=htmlentities(upperpath($op))?>" href="ns_imgbrowser.php?op=<?=rawurlencode(upperpath($op))?>"><img src="images/folderclosed.gif" border="0"></a></td>
				<td nowrap><a title="<?=htmlentities(upperpath($op))?>" href="ns_imgbrowser.php?op=<?=rawurlencode(upperpath($op))?>">..</a></td>
			</tr>
		</table></td>
	</tr>
<? } ?>
<? for ($i=0;$i<count($arr_dir);$i++) { ?>
	<tr>
		<td><table cellpadding="0" cellspacing="0" border="0">
			<tr>
				<td nowrap><a title="<?=htmlentities($op.$arr_dir[$i]['name']."/")?>" href="ns_imgbrowser.php?op=<?=rawurlencode($op.$arr_dir[$i]['name']."/")?>"><img src="images/folderclosed.gif" border="0"></a></td>
				<td nowrap><a title="<?=htmlentities($op.$arr_dir[$i]['name']."/")?>" href="ns_imgbrowser.php?op=<?=rawurlencode($op.$arr_dir[$i]['name']."/")?>"><?=str_trunc($arr_dir[$i]['name'],20)?></a></td>
			</tr>
		</table></td>
	</tr>
<? } ?>
