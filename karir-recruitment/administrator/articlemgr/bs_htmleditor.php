...
function BS_HTMLEDIT($fieldname,$formname,$content,$width,$height,$inc,$toolbar,$baseurl,$body_attr) {
	...
	$bs_num++;
	if (!is_array($toolbar)) $toolbar = array("undo","redo","cut","copy","paste","find","bold","italic","underline","justifyleft",
	...
	$IsMSIE = strpos(strtoupper($_SERVER["HTTP_USER_AGENT"]),"MSIE") && strpos(strtoupper($_SERVER["HTTP_USER_AGENT"]),"WIN");
	if ($IsMSIE) {
