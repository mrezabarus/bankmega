...
<html>
<head>
	<title><?=$_FLM["TITLE"]?> | Move File</title>
  ...
  <!--
  	...
  	function frmSubmit() {
			frm = document.frmDirTree;
			frm.action = "qmove.php?<?=$_instanceurl?>";
			<? if ($selpath != $path) { ?>
				frm.submit();
			<? } else { ?>
				alert('Cannot move file, source and destination folder are the same!')
			<? } ?>
		}
  //-->
  </SCRIPT>
...
<table cellpadding="0" cellspacing="1" border="0" width="100%">
  ...
  <tr>
    <td class="flm_td_title1">
			<table cellpadding="2" width="100%">
				<tr>
					<td class="flm_td_title1" align="left"><b>Move File(s)</b></td>
				</tr>
			</table>
		</td>
  </tr>
  ...
  <tr>
    <td align="left">
			<table cellpadding="1">
				<tr>
					<td><input type="button" class="flm_button" value="       Move!       " onClick="frmSubmit(); return false;"> <input class="flm_button" type="button" value="Back to File List!" onClick="location = 'filelist.php?PATH=<?=rawurlencode($path)?>&<?=$_instanceurl?>&SEED=<?=mktime()?>'"></td>
				</tr>
			</table>
		</td>
  </tr>
