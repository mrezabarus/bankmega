	...
	$ctr = 0;
		...
		foreach ($files as $el) {
			...
			if (_flm_invalidfname($el) || !_flm_isvalid_ext($pinfo_new["extension"]) || !_flm_isvalid_file($el)) {
				$arrResult[$el] = "<font color=\"#ff0000\">File cannot be copied!</font>";
			} elseif (@is_file($_FLM["ROOTPATH"].$selpath.$el)) {
				$arrResult[$el] = "<font color=\"#ff0000\">File already exists!</font>";
			} elseif ($path == $selpath) {
				$arrResult[$el] = "<font color=\"#ff0000\">Cannot copy file, source and destination folder are the same!</font>";
			} else {
				$copy_result = @copy($_FLM["ROOTPATH"].$path.$el,$_FLM["ROOTPATH"].$selpath.$el);
				if ($copy_result) $arrResult[$el] = "File is copied!";
				else $arrresult[$el] = "<font color=\"#ff0000\">Cannot copy file, unknown error occured while processing request!</font>";
			}
		}
...
<html>
<head>
	<title><?=$_FLM["TITLE"]?> | Copy File(s) - Result!</title>
  ...
  <tr>
    <td class="flm_td_title1">
			<table cellpadding="2" width="100%">
				<tr>
					<td class="flm_td_title1" align="left"><b>Copy File(s) - Result!</b></td>
				</tr>
			</table>
		</td>
  </tr>
