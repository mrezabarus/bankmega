...
class udv_db {
	...
	function udv_db($db_name_rec = "",$db_name ="",$db_host = "",$db_user = "",$db_pwd = "") {
		$this->db_name_rec = $db_name_rec;
	...
	function connect() {
		if ($this->cnn == 0) {
			...
			if (!@mysql_select_db($this->db_name)) {
				...
				return 0;
			}elseif (!@mysql_select_db($this->db_name_rec)){
			 				$this->err_handler("Cannot select database!");
				return 0;
          }
		}
  ...
  function free() {
	...
	function next() {
    if (!$this->rs) {
      $this->err_handler("Cannot move data to next record!");
      return 0;
    }
